...
async def create_assignment_with_file(
    ...
    submission_type: Optional[str] = Form("autoComplete"),
    ...
    new_assignment = Assignment(
        ...
        due_date=due_date,
        teacher_comments=teacher_comments or "",
    ...
    assignment_dict = {
        ...
        "is_completed": new_assignment.is_completed,
        "submission_type": new_assignment.submission_type,
        "files": [],
    }
...
async def get_assignment(
    ...
    # If user is a student, get their progress
    if user.role == "student":
        ...
        if progress:
            assignment_dict["is_completed"] = progress.is_completed
...
async def get_assignment_with_progress(
    ...
    if progress:
        # Calculate is_completed based on status
        is_completed = progress.status in [AssignmentStatus.COMPLETED, AssignmentStatus.GRADED]
...
@router.get("", response_model=List[AssignmentWithProgressResponse])
async def get_course_assignments(
    ...
    if section_id is not None:
        # Special case: section_id=0 means assignments without a section
        if section_id == 0:
    ...
    for assignment in assignments:
        # Get progress for this assignment if the user is a student
        progress = None
        if not (is_teacher or is_admin):
            progress = (
                db.query(AssignmentProgress)
...
async def delete_assignment(
    ...
    # Keep track of which students had this assignment completed
    students_to_update = set()
    
    # First, delete associated AssignmentProgress records
    try:
        ...
        # Save student IDs that had progress records for this assignment
        for progress in progress_records:
            students_to_update.add(progress.student_id)
            
            # Special case: if the assignment was completed, we'll need to update the count
        ...
        db.flush()  # Flush changes to prevent potential conflicts
    except Exception as e:
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error deleting assignment progress records: {str(e)}",
        )

    # Delete associated files from S3
    try:
        # List and delete all files in this assignment's folder
        ...
        print(f"Error deleting S3 files for assignment {assignment_id}: {str(e)}")
    ...
    try:
        ...
        # Update all student progress records for this course
        from backend.controllers.progress import update_course_progress
        
        # If no students had progress records for this specific assignment,
        # update all enrolled students' progress records
        if not students_to_update:
            for student in course.students:
                students_to_update.add(student.id)
        ...
        for student_id in students_to_update:
            try:
                ...
                print(f"Error updating progress for student {student_id}: {str(e)}")
